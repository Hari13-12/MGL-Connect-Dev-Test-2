...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("last_login_at", sa.DateTime(timezone=True)), schema="app",
    )
    op.create_table(
        "otp_request",
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("now()"), nullable=False), schema="app",
    )
    op.create_index("ix_app_otp_request_destination_hash", "otp_request", ["destination_hash"], schema="app")
    op.create_table(
        "user_access",
        sa.Column("user_access_id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("app.app_user.user_id"), nullable=False),
        ...
        sa.UniqueConstraint("user_id", "service_contract_sfid", "account_sfid", name="uq_user_access_scope"), schema="app",
    )


def downgrade() -> None:
    op.drop_table("user_access", schema="app")
    op.drop_index("ix_app_otp_request_destination_hash", table_name="otp_request", schema="app")
