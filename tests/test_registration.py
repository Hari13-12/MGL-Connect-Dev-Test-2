...
async def test_valid_registration_creates_only_validated_access(session):
    otp = FakeOtpAdapter()
    settings = Settings()
    request_id = await start_registration(
        registration(), FakeSalesforceAdapter(), otp, MemoryRateLimiter(), session, settings
    )
    result = await verify_registration(
        VerifyIn(otp_request_id=request_id, otp="123456"), otp, session, settings
    )
    user = await session.get(AppUser, result)
    access = (await session.execute(select(UserAccess))).scalar_one()
...
async def test_invalid_customer_and_otp_never_create_user(session):
    with pytest.raises(Exception):
        await start_registration(
            registration().model_copy(update={"ca_number": "wrong"}),
            FakeSalesforceAdapter(),
            FakeOtpAdapter(),
            MemoryRateLimiter(),
            session,
            Settings(),
    ...
    with pytest.raises(Exception):
        await verify_registration(
            VerifyIn(otp_request_id=request_id, otp="000000"), otp, session, Settings()
        )
...
async def test_expired_otp_is_rejected(session):
    ...
    row = await session.get(OtpRequest, request_id)
