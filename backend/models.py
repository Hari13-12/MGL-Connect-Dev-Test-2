...
class OtpRequest(Base):
    ...
    status: Mapped[OtpStatus] = mapped_column(
        Enum(OtpStatus), default=OtpStatus.PENDING, nullable=False
    )
...
class UserAccess(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "service_contract_sfid", "account_sfid"),
        Index("ix_user_access_contract_account", "service_contract_sfid", "account_sfid"),
    )
    user_access_id: Mapped[str] = mapped_column(
        String(36), primary_key=True, default=lambda: str(uuid4())
    )
    user_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("app_user.user_id"), nullable=False, index=True
    )
    service_contract_sfid: Mapped[str] = mapped_column(String(64), nullable=False)
    account_sfid: Mapped[str] = mapped_column(String(64), nullable=False)
