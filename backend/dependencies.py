...
@lru_cache
def otp_adapter(): return FakeOtpAdapter()
@lru_cache
def salesforce_fake(): return FakeSalesforceAdapter()
@lru_cache
def rate_limiter(): return MemoryRateLimiter()
async def customer_adapter(settings: Settings=Depends(get_settings), session=Depends(get_session)):
