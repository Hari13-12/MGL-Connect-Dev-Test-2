...
class FakeSalesforceAdapter:
    ...
    async def validate(self, bp, ca, mobile, email):
        for row in self.records:
            if (row["bp"], row["ca"], row["mobile"], row["email"].lower()) == (
                bp,
                ca,
                mobile,
                email.lower(),
...
class MirrorSalesforceAdapter:
    ...
    def __init__(self, session: AsyncSession, settings: Settings):
        self.session, self.s = session, settings

    async def validate(self, bp, ca, mobile, email):
        s = self.s
        # Configuration names are deployment-controlled identifiers; values remain bound parameters.
        query = text(
            f"SELECT service_contract_sfid, account_sfid FROM {s.salesforce_contract_table} WHERE {s.salesforce_bp_field}=:bp AND {s.salesforce_ca_field}=:ca AND {s.salesforce_mobile_field}=:mobile AND {s.salesforce_email_field}=:email"
        )
        row = (
            (
                await self.session.execute(
                    query, {"bp": bp, "ca": ca, "mobile": mobile, "email": email}
                )
            )
            .mappings()
            .first()
...
def destination_digest(mobile: str) -> str:
